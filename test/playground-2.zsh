...
for ITEM in {integer,float,scalar,array}; do
	...
	case "$ITEM" in
		...
		integer|float|scalar)
			ITEMP+="JSON_${ITEMC}_VALUE=" ;|
		integer|float) ITEMP+='0'
		array)   ITEMP="-a json_array_value=( )" ;;
	esac
	alias "json:value:switch:$ITEM"="local $ITEMP; json-value-switch-$ITEM "$@"; argv=( \"\$argv_result\" )"
done
...
function json-scalar{,-lines}() {
	...
	(( ${#lines[@]} == 1 )) || { json-array $(( $1 + 1 )) lines "${lines[@]}"; return $? }
	json_scalar -v "$JSON_SCALAR_VALUE" "$@"
}
function json-array() {
	(( $# >= 2 )) || die "$0: Expected at least 2 parameters - indent and json-name"
	json-varname $1 "$2"
...
function a() {
	function b() {
		print-it association parameters
